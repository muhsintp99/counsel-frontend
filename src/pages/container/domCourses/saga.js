...
import * as actions from './domisticCoursesSlice'; // updated import
...
function* addCourseSaga(action) {
  try {
    const {
      title, shortDescription, fullDescription, duration,
      category, mode, fees, image, syllabus, prerequisites,
      tags, visible, createdBy, updatedBy
    } = action.payload;
...
function* updateCourseSaga(action) {
  try {
    const {
      _id, title, shortDescription, fullDescription, duration,
      category, mode, fees, image, syllabus, prerequisites,
      tags, visible, updatedBy
    } = action.payload;

    if (!_id) throw new Error('Missing course ID in payload');
...
// Watcher
export default function* DomesticCoursesWatcher() {
  yield takeEvery('domisticCourses/getCourse', getCourseSaga);
  yield takeEvery('domisticCourses/getCourseById', getCourseByIdSaga);
  yield takeEvery('domisticCourses/addCourse', addCourseSaga);
  yield takeEvery('domisticCourses/updateCourse', updateCourseSaga);
  yield takeEvery('domisticCourses/deleteCourse', deleteCourseSaga);
  yield takeEvery('domisticCourses/totalCourseCount', totalCourseCountSaga);
}
